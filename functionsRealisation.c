...
struct customersInfo addCustomers(struct customersInfo database)
{
    ...
    for (size_t i = database.amount - number; i < database.amount; ++i)
    {
        ...
        } while (!(database.customers[i].address.houseNumber = getUnsigned()) || database.customers[i].address.houseNumber > 999);
        do
        {
            system("cls");
...
void showCustomers(struct customersInfo database)
{
    ...
    for (size_t i = 0; i < database.amount; ++i)
    {
        size_t max = surnameColumnLen + nameColumnLen + patronymicColumnLen + addressColumnLen + phoneNumberLen + cardNumberLen + 22;
